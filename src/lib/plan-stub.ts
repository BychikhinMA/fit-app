...
export async function generateStubPlan(profileId: ProfileId, answers: OnboardingAnswers) {
  ...
  const { error: settingsError } = await supabase.from('profile_settings').upsert({
    ...
    bmi_calculated_at: now,
    calorie_method: calorieRecommendation.method,
    recommended_calories: calorieRecommendation.recommended_calories,
    calorie_protein_g: calorieRecommendation.protein_g,
    calorie_fat_g: calorieRecommendation.fat_g,
    calorie_carbs_g: calorieRecommendation.carbs_g,
    calorie_calculated_at: now,
  ...
  const { data: mealPlan, error: mealPlanError } = await supabase
    ...
    .insert({
      profile_id: profileId,
      onboarding_response_id: onboardingResponse.id,
      daily_calories: calorieRecommendation.recommended_calories,
      protein_g: calorieRecommendation.protein_g,
      fat_g: calorieRecommendation.fat_g,
      carbs_g: calorieRecommendation.carbs_g,
  ...
  const meals = mealSplit.map((m, i) => ({
    ...
    calories: Math.round(calorieRecommendation.recommended_calories * m.share),
    protein_g: Math.round(calorieRecommendation.protein_g * m.share),
    fat_g: Math.round(calorieRecommendation.fat_g * m.share),
    carbs_g: Math.round(calorieRecommendation.carbs_g * m.share),
    sort_order: i,
  }));
