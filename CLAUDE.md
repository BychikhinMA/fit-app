...
# Текущее состояние проекта (обновлено 2026-09-21)
...
- Календарь v2 на вкладке «Тренировки» (2026-09-21, коммит `05b3af1`):
  недельная сетка из предыдущей сессии заменена на полноценный переключатель
  масштаба День/Неделя/Месяц/Квартал/Год с навигацией вперёд/назад и кнопкой
  «Сегодня» (`src/components/calendar/scale-switcher.tsx` + пять
  вид-компонентов `day-view.tsx`/`week-view.tsx`/`month-view.tsx`/
  `quarter-view.tsx`/`year-view.tsx`, дата-математика в
  `src/lib/calendar-dates.ts`). Основа — новая колонка
  `workout_days.weekday` (0=Пн..6=Вс, nullable; миграция
  `supabase/migrations/0006_workout_day_weekday.sql`, применена в Dashboard
  и перепроверена сквозным способом: SQL-миграция выполнена через SQL
  Editor, затем прямым `select` подтверждено, что `weekday` у обеих
  существующих программ проставлен верно и сбрасывается на 0 для каждой
  программы отдельно) — план считается бессрочным и повторяющимся
  еженедельно, конкретные календарные даты нигде не хранятся, клиент сам
  сопоставляет дату дню недели (`findWorkoutForDate`). `plan-stub.ts`
  проставляет `weekday` для новых дней. В Квартале/Году кликабелен целиком
  весь мини-месяц (`month-mini.tsx`, переключает на Месяц), не отдельный
  день — так задумано, чтобы не городить нереалистичное число 44pt-таргетов
  на экран. Заодно на `workout-day/[id].tsx` добавлен ручной переключатель
  контекста тренировки (напр. Дом/Зал) поверх уже существующего поля
  `exercises.context_variants`, переиспользует `Chip` из
  `components/onboarding/chip.tsx`. **Известное ограничение:** переключатель
  контекста не проявляется на текущих тестовых данных — у профилей Максима
  и Марии в `training_contexts` заведён только один контекст, поэтому
  условие показа (`contexts.length > 1`) скрывает панель; логика проверена
  и корректна, но живьём с реальным переключением между >1 контекстом не
  протестирована — понадобится профиль с двумя+ контекстами в онбординге
  (напр. «Дом» и «Зал» одновременно), чтобы увидеть панель на экране.
  Живая QA в браузере (Claude in Chrome) пройдена по всем пяти масштабам,
  клику по дню/мини-месяцу, кнопке «Сегодня», узкой ширине экрана (390px) —
  без ошибок в консоли; `npx tsc --noEmit` и `eslint` по изменённым файлам
  чистые.
