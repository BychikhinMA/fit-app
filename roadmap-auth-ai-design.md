...
**СТАТУС: РЕАЛИЗОВАНО** (2026-09-02, коммит `5e9e25e`, влит в `main`,
проверен вживую). Токены акцента добавлены в `src/constants/theme.ts` и
точечно применены там, где состояние раньше показывалось чёрной заливкой
или не выделялось вовсе: выбранный чип онбординга (`chip.tsx`), главная
кнопка онбординга (`onboarding/index.tsx`), активный пункт навигации
(`tab-trigger-button.tsx` — только тинт иконки), `linkPrimary` в
`ThemedText`, CTA «Выбрать это упражнение» (`exercise-library/[id].tsx`).

Отклонения от плана ниже (согласованы в ходе реализации ради контраста
по WCAG):

1. **`onAccent` в светлой теме — тёмный (`#1A1A1A`), не белый.** Белый на
   `#FF7A1A` даёт всего 2.6:1 — не проходит даже порог 3:1 для крупного
   текста; `#1A1A1A` на той же оранжевой даёт ~6.7:1. В тёмной теме
   `onAccent` остаётся `#000000` на более светлой оранжевой.
2. **Оранжевый не используется как цвет текста на светлом фоне.** В
   навигации оранжевый — только тинт иконки активного пункта, подпись
   остаётся `theme.text`.
3. **Отдельный токен `accentText`** (`#C2410C` на белом ≈ 5.2:1 /
   `#FDBA74` в тёмной теме) для акцентного *текста* (`linkPrimary` и
   т.п.) вместо самого `accent`.
4. **`backgroundElement` осветлён до `#F5F5F5`**; `backgroundSelected`
   стал светло-оранжевой подложкой (`#FFE4CC` / `#3A2412`).
5. Попутно исправлен баг: неактивная главная кнопка онбординга рисовала
   белую подпись на светлом фоне (~1.05:1, текст был невидим) — теперь
   фон `backgroundElement`, подпись `textSecondary`. Кнопки выбора
   профиля/цели и кнопка «Заменить» на экране дня оставлены на
   нейтральном фоне (accent для них — перебор).

Итоговые значения токенов — в `src/constants/theme.ts`. Ниже — исходный
набросок из плана; значения `onAccent` и отсутствие `accentText` в нём
отличаются от того, что в итоге влито (см. отклонения выше):
